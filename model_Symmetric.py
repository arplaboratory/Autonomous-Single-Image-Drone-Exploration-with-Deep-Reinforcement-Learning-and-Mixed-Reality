...
class CriticIMPALA(nn.Module):
    def __init__(self, settings):
        ...
        # track
        self.features_RGB_track = FeaturesExtractor_RGB(settings)
        self.RGB_shape_track = self.features_RGB_track.feat_shape
        # -
        self.lin_track = nn.Linear(self.RGB_shape_track, self.dl)
        ...
        # # exp
        # self.features_RGB_exp = FeaturesExtractor_RGB(settings)
        # self.RGB_shape_exp = self.features_RGB_exp.feat_shape
        # # -
        # self.lin_exp = nn.Linear(self.RGB_shape_exp, self.dl)
    ...
    def forward(self, input_data):
        ...
        featuresRGB_track = self.features_RGB_track.forward(input_data).reshape(-1, self.RGB_shape_track)
        # featuresRGB_exp = self.features_RGB_exp.forward(input_data).reshape(-1, self.RGB_shape_exp)

        # track
        x = F.relu(self.lin_track(featuresRGB_track))
        ...
        # # exp
        # x = F.relu(self.lin_exp(featuresRGB_exp))
        # x_ = x.view(input_shape[0], -1, self.dl)
        ...
        return values_track, values_track, track_h_c_, track_h_c_
