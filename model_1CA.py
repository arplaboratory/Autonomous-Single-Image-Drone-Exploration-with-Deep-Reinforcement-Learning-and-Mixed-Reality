...
class ActorIMPALA(nn.Module):
    def __init__(self, settings):
        ...
        #self.features_RGB = FeaturesExtractorResNet(settings)
        self.RGB_shape = self.features_RGB.feat_shape

        # Linear
        self.lin1 = nn.Linear(self.RGB_shape, 750)      # 3
    ...
    def forward(self, input_data):
        yaw = input_data['yaw'].reshape(-1, 1)
        ...
        #featuresRGB = torch.cat([yaw, angle, distance], dim=-1)
...
class CriticIMPALA(nn.Module):
    def __init__(self, settings):
        ...
        # -
        self.feat_shape = self.Grid_shape + self.Ego_shape
        self.lin = nn.Linear(self.feat_shape + 400, self.dl)  # + 400
        self.gru = nn.GRU(self.dl, self.dr)
        # hidden
...
class ActorCriticIMPALA(nn.Module):
    def __init__(self, settings):
        ...
        self.hidden0_exp = nn.Linear(self.dr, 256)
        self.hidden1_exp = nn.Linear(256, 128)
        self.output_exp = nn.Linear(128, self.settings['output_a'] + 1)
    ...
    def forward(self, input_data):
        ...
        return values_exp, values_exp, logits_track, logits_exp, h_a_
...
class IMPALA_Net(nn.Module):
    def __init__(self, settings):
        ...
        self.actor, self.critic = ActorIMPALA(settings), CriticIMPALA(settings)
    ...
    def forward(self, input_data):
        ...
        output_data = {'logits_track': logits_track,
                       ...
                       'values_track': torch.tensor(0),  # values_track,
                       'values_exp': torch.tensor(0),  # values_exp,
