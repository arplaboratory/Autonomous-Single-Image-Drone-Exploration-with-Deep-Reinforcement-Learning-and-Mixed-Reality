...
class MyLearner(mp.Process):
    ...
    def run(self):
        ...
        if self.settings['WandB']:
            if self.settings['IMPALA']:
                wandb.init(project="NYU_EXP_{}".format(self.settings['workstation_name']), entity='pegaso', name='IMPALA_baseline', config=self.settings)
    ...
    def update(self):
        ...
        while True:
            ...
            samples = self.memory_out_socket.client_receive()  # TODO: data my_agent

            if self.model_update % 100 == 0:
                # self.fps = 100 * self.settings['update_settings']['batch_size'] * \
                #            self.settings['update_settings']['data_keys']['reward'] / (
    ...
    def save_log(self, training_log):
        if self.settings['WandB']:
            # training_log['train/learner_fps'] = self.fps
            try:
    ...
    def on_policy_update(self, agent_state_dict, memory_id, socket_id):
        ...
        # assert self.settings['update_settings']['mode'] == 'newest'
