...
def get_input_data_DDPG(samples, device):
    actions = samples['action']
    rewards = samples['reward']
    done = samples['done']
    camera = samples['RGBCamera']

    next_camera = samples['next_state']
    ...
    next_camera = torch.from_numpy(next_camera).float().to(device)

    new_stack_camera = torch.cat((camera[1:], next_camera))

    input_data = dict()
    input_data['rewards'] = rewards
    input_data['done'] = done
    input_data['get_critic'] = {'RGBCamera': camera, 'action': actions}
    input_data['get_action_critic_target'] = {'RGBCamera': new_stack_camera}
    input_data['get_action'] = {'RGBCamera': camera}
    return input_data
...
class Belief(object):
    ...
    def measurement_model(self, sensor_map, obstacle_map):
        ...
        tracker_confidence = torch.tensor([0.9]).to(self.device)  # TODO
        sensor_thr = 0.1  # TODO
        ...
        grid_t_nt = torch.where(sensor_map[2] > 0.8, torch.tensor([0.1]).to(self.device), grid).to(self.device)
        ...
        grid_t_t = torch.where(grid_t_t == 0.0, torch.tensor([1.0]).to(self.device), grid_t_t).to(self.device)
        grid_t_nt = torch.where(grid_t_nt == 0.0, torch.tensor([1.0]).to(self.device), grid_t_nt).to(self.device)
...
class ScoreCounter:
    def __init__(self, dis_exp, dis_max):
        self.scores = {'score_angle': 0., 'score_distance': 0., 'score_total': 0., 'score_perc': 0.,
                       'target_founded': 0, 'coverage_perc': 0.}
    ...
    def add_score(self, info):
        ...
        if score_total > 0:
            ...
            self.target_founded = True
        else:
            self.check_list['exp_steps'] += 1

        if self.target_founded:
            self.check_list['effective_track_steps'] += 1

        self.scores['score_angle'] += round(score_angle, 3)
        self.scores['score_distance'] += round(score_distance, 3)
        self.scores['score_total'] += round(score_total, 3)
        self.scores['score_perc'] += round(score_perc, 3)

    def reset(self):
        if self.target_founded:
            self.scores['target_founded'] += 1
    ...
    def save_scores(self, ep, st, tm, name):
        ...
        f = open('C:\\Users\\Perseo\\Documents\\ActiveTracking\\experiments\\experiment_2TF_' + timestr + '.txt', 'w')
        ...
        f.write('Score Angle:    {:<20}Score Angle Mean Total:    {:<20}Score Angle Mean Track:    {:<20}'
                'Score Angle Mean Effective Track:    {:<20}\n\n'.format(
                 round(self.scores['score_angle'], 3),
                 round(self.scores['score_angle']/self.check_list['total_steps'], 3),
                 round(self.scores['score_angle']/self.check_list['track_steps'], 3)
                       if self.check_list['track_steps'] > 0 else 0.,
                 round(self.scores['score_angle']/self.check_list['effective_track_steps'], 3)
                       if self.check_list['effective_track_steps'] > 0 else 0.))

        f.write('Score Distance: {:<20}Score Distance Mean Total: {:<20}Score Distance Mean Track: {:<20}'
                'Score Distance Mean Effective Track: {:<20}\n\n'.format(
                 round(self.scores['score_distance'], 3),
                 round(self.scores['score_distance'] / self.check_list['total_steps'], 3),
                 round(self.scores['score_distance'] / self.check_list['track_steps'], 3)
                       if self.check_list['track_steps'] > 0 else 0.,
                 round(self.scores['score_distance'] / self.check_list['effective_track_steps'], 3)
                       if self.check_list['effective_track_steps'] > 0 else 0.))

        f.write('Score Total:    {:<20}Score Total Mean Total:    {:<20}Score Total Mean Track:    {:<20}'
                'Score Total Mean Effective Track:    {:<20}\n\n'.format(
                 round(self.scores['score_total'] / 2, 3),
                 round(self.scores['score_total'] / self.check_list['total_steps'] / 2, 3),
                 round(self.scores['score_total'] / self.check_list['track_steps'] / 2, 3)
                       if self.check_list['track_steps'] > 0 else 0.,
                 round(self.scores['score_total'] / self.check_list['effective_track_steps'] / 2, 3)
                       if self.check_list['effective_track_steps'] > 0 else 0.))

        f.write('Score Perc:     {:<20}Score Perc Mean Total:     {:<20}Score Perc Mean Track:     {:<20}'
                'Score Perc Mean Effective Track:     {:<20}\n\n'.format(
                 round(self.scores['score_perc'], 3),
                 round(self.scores['score_perc'] / self.check_list['total_steps'], 3),
                 round(self.scores['score_perc'] / self.check_list['track_steps'], 3)
                       if self.check_list['track_steps'] > 0 else 0.,
                 round(self.scores['score_perc'] / self.check_list['effective_track_steps'], 3)
                       if self.check_list['effective_track_steps'] > 0 else 0.))
