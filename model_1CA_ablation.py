...
class CriticIMPALA(nn.Module):
    def __init__(self, settings):
        ...
        #self.lin_dis_angle_hit = nn.Linear(3, 300)             # WITH MAPS
        ...
        #self.feat_shape = self.Grid_shape + self.Ego_shape     # WITH MAPS
        self.feat_shape = self.Ego_shape                        # NO MAPS
        #self.lin = nn.Linear(self.feat_shape + 400, self.dl)   # WITH MAPS
        self.lin = nn.Linear(self.feat_shape + 100, self.dl)    # NO MAPS
        self.gru = nn.GRU(self.dl, self.dr)
        # hidden
    ...
    def forward(self, input_data):
        ...
        #featuresGrid = self.features_Grid.forward(input_data).reshape(-1, self.Grid_shape)     # WITH MAPS
        featuresEgo = self.features_Ego.forward(input_data).reshape(-1, self.Ego_shape)

        #dis_angle_hit = torch.cat((distance, angle, hit), dim=1)           # WITH MAPS
        #x_dist_angle_hit = F.relu(self.lin_dis_angle_hit(dis_angle_hit))   # WITH MAPS
        #features = torch.cat((featuresGrid, featuresEgo), dim=1)           # WITH MAPS
        features = featuresEgo                                              # NO MAPS
        x_yaw = F.relu(self.lin_yaw(input_yaw))

        #x_cat = torch.cat((features, x_yaw, x_dist_angle_hit), dim=1)      # WITH MAPS
        x_cat = torch.cat((features, x_yaw), dim=1)  # WITH MAPS
...
class IMPALA_Net(nn.Module):
    ...
    def forward(self, input_data):
        ...
        output_data = {'logits_track': logits_track,
                       ...
                       'track_h_c_': track_h_c_,
                       'exp_h_c_': exp_h_c_,                    # TODO 'h_c_': h_c_
                       'fov': fov}
