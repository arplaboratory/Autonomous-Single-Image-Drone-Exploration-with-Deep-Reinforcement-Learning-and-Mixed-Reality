...
class FeaturesExtractor_Grid(nn.Module):
    def __init__(self, settings):
        ...
        self.conv1 = nn.Conv2d(in_channels=2, out_channels=self.d1,
                               kernel_size=4, stride=2, padding=2)
        self.bnc1 = torch.nn.GroupNorm(int(self.d1 / 2), self.d1)
        self.conv2 = nn.Conv2d(in_channels=self.d1, out_channels=self.d1,
    ...
    def forward(self, input_data):
        ...
        input_data['bel'] = input_data['bel'].reshape(-1, 3, 35, 35)
        inp = input_data['bel'][:, :2].reshape(-1, 2, self.grid, self.grid).to(self.device)
...
#
#         # #################-tail_track-#############################
#         self.hidden0_track = nn.Linear(self.dr, 128)
#         self.hidden1_track = nn.Linear(128, 64)
#         self.output_track = nn.Linear(64, 1)
#
#         # ##################-tail_exp-##############################
#         self.hidden0_exp = nn.Linear(self.dr, 128)
#         self.hidden1_exp = nn.Linear(128, 64)
#         self.output_exp = nn.Linear(64, 1)
#
#     def forward(self, input_data):
#         input_shape = input_data['RGBCamera'].shape
#         input_hc = input_data['track_h_c']  # TODO input_hc = input_data['h_c']
...
#         xh0_track = F.relu(self.hidden0_track(xgru))
#         xh1_track = F.relu(self.hidden1_track(xh0_track))
#         values_track = self.output_track(xh1_track)
#
#         # ##################-tail_exp-##############################
#         xh0_exp = F.relu(self.hidden0_exp(xgru))
#         xh1_exp = F.relu(self.hidden1_exp(xh0_exp))
#         values_exp = self.output_exp(xh1_exp)
#
#         return values_track, values_exp, h_c_


# #############################################-TWO_TAILS_EXPERIMENTS_CRITIC-###########################################
# class CriticIMPALA(nn.Module):
#     def __init__(self, settings):
#         super(CriticIMPALA, self).__init__()
#         self.settings = settings
...
#         self.lin_dis_angle_hit = nn.Linear(3, 300)
#         # -
#         self.lin_track = nn.Linear(300, self.dl)
#         self.gru_track = nn.GRU(self.dl, self.dr)
#         # hidden
#         self.hidden1_track = nn.Linear(self.dr, 128)
#         self.hidden2_track = nn.Linear(128, 64)
#         self.output_track = nn.Linear(64, 1)
#
#         # exp
#         self.features_Grid = FeaturesExtractor_Grid(settings)
...
#         # -
#         self.feat_shape = self.Grid_shape + self.Ego_shape
#         self.lin_exp = nn.Linear(self.feat_shape + 100, self.dl)
#         self.gru_exp = nn.GRU(self.dl, self.dr)
#         # hidden
#         self.hidden1_exp = nn.Linear(self.dr, 128)
#         self.hidden2_exp = nn.Linear(128, 64)
#         self.output_exp = nn.Linear(64, 1)
#
#     def forward(self, input_data):
#         input_shape = input_data['RGBCamera'].shape
...
#         dis_angle_hit = torch.cat((distance, angle, hit), dim=1)
#         x_dist_angle_hit = F.relu(self.lin_dis_angle_hit(dis_angle_hit))
#         x = F.relu(self.lin_track(x_dist_angle_hit))
#         x_ = x.view(input_shape[0], -1, self.dl)
#         xgru, track_h_c_ = self.gru_track(x_, input_track_hc)
#         xh1 = F.relu(self.hidden1_track(xgru))
#         xh2 = F.relu(self.hidden2_track(xh1))
#         values_track = self.output_track(xh2)
#
#         # exp
#         features = torch.cat((featuresGrid, featuresEgo), dim=1)
#         x_yaw = F.relu(self.lin_yaw(input_yaw))
#         x_cat = torch.cat((features, x_yaw), dim=1)
#         x = F.relu(self.lin_exp(x_cat))
#         x_ = x.view(input_shape[0], -1, self.dl)
#         xgru, exp_h_c_ = self.gru_exp(x_, input_exp_hc)
#         xh1 = F.relu(self.hidden1_exp(xgru))
#         xh2 = F.relu(self.hidden2_exp(xh1))
#         values_exp = self.output_exp(xh2)
#
#         return values_track, values_exp, track_h_c_, exp_h_c_
######################################################################################################################
...
class CriticIMPALA(nn.Module):
    def __init__(self, settings):
        ...
        self.lin_dis_angle_hit = nn.Linear(3, 300)
        # self.features_Grid = FeaturesExtractor_Grid(settings)  # TODO c
        # self.Grid_shape = self.features_Grid.feat_shape  # TODO c
        self.features_Ego = FeaturesExtractor_Ego(settings)
        self.Ego_shape = self.features_Ego.feat_shape
        self.lin_yaw = nn.Linear(1, 100)
        # -
        # self.feat_shape = self.Grid_shape + self.Ego_shape  # TODO c
        self.feat_shape = self.Ego_shape  # TODO u
        self.lin = nn.Linear(self.feat_shape + 100 + 300, self.dl)
        self.gru = nn.GRU(self.dl, self.dr)
        # hidden
    ...
    def forward(self, input_data):
        ...
        angle = input_data['angle'].reshape(-1, 1)
        hit = input_data['hit'].reshape(-1, 1)
        # featuresGrid = self.features_Grid.forward(input_data).reshape(-1, self.Grid_shape)  # TODO c
        featuresEgo = self.features_Ego.forward(input_data).reshape(-1, self.Ego_shape)

        dis_angle_hit = torch.cat((distance, angle, hit), dim=1)
        x_dist_angle_hit = F.relu(self.lin_dis_angle_hit(dis_angle_hit))
        # features = torch.cat((featuresGrid, featuresEgo), dim=1)  # TODO c
        features = featuresEgo  # TODO c
        x_yaw = F.relu(self.lin_yaw(input_yaw))
        x_cat = torch.cat((features, x_yaw, x_dist_angle_hit), dim=1)
        x = F.relu(self.lin(x_cat))
        x_ = x.view(input_shape[0], -1, self.dl)
...
class IMPALA_Net(nn.Module):
    ...
    def forward(self, input_data):
        ...
        output_data = {'logits_track': logits_track,
                       ...
                       'track_h_c_': track_h_c_, 'exp_h_c_': exp_h_c_,                    # TODO 'h_c_': h_c_
                       'fov': fov}
